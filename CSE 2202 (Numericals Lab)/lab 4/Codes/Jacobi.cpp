...
void jacobi(vector<vector<double>> &mat, double error, double x, double y, double z, vector<double> &ans){
    ...
    while(ex>=error || ey>=error || ez>=error){
        ...
        y1 = (mat[1][3]-z0*mat[1][2]-x0*mat[1][0])/mat[1][1];
        z1 = (mat[2][3]-x0*mat[2][0]-y0*mat[2][1])/mat[2][2];
